...
def evaluate_clustering(X, labels_true, labels_pred, clus_algo_name, dataset_name, results_path, algorithm_details,
    ...
    if len(unique_labels) > 1:
        ...
        calinski_harabasz = np.nan  # Use NaN or a similar placeholder to indicate the metric couldn't be calculated
...
DATASET_DIRS = ['A-sets', 'Birch-sets', 'DIM-sets-high', 'G2-sets', 'S-sets', 'Unbalance']
# DATASET_DIRS = ['A-sets']
metrics_file_path = os.path.join(current_directory, 'results', 'metrics')
# Determine the next version number automatically


# algorithms = ['KMeans', 'DBSCAN', 'AffinityPropagation', 'BIRCH',  'OPTICS', 'Mean Shift', 'AgglomerativeClustering']
algorithms = ['KMeans']
